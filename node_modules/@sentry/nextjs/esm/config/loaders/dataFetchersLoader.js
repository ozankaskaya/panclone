...
import { isESM } from '../../utils/isESM.js';
import { makeAST, removeComments, renameIdentifiers, findExports, findDeclarations, getExportIdentifierNames, hasDefaultExport } from './ast.js';
...
function wrapFunctions(userCode, templateCode, filepath) {
  ...
  for (var functionName of Object.keys(DATA_FETCHING_FUNCTIONS)) {
    ...
    var alias = renameIdentifiers(userAST, functionName);
...
function wrapDataFetchersLoader( userCode) {
  ...
  var parameterizedRouteName = path
  ...
  if (!this.resourceQuery.includes('sentry-proxy-loader')) {
    var ast = makeAST(userCode);
    var exportedIdentifiers = getExportIdentifierNames(ast);
    ...
    var templatePath = path.resolve(__dirname, '../templates/dataFetchersLoaderTemplate.js');
    // make sure the template is included when runing `webpack watch`
    this.addDependency(templatePath);

    var templateCode = fs.readFileSync(templatePath).toString();
