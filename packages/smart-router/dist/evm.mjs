...
// evm/getBestTrade.ts
import { TradeType as TradeType5 } from "@pancakeswap/sdk";

// evm/getBestTradeFromStablePools.ts
import { Price as Price2, TradeType as TradeType2 } from "@pancakeswap/sdk";
...
import { CurrencyAmount as CurrencyAmount5 } from "@pancakeswap/sdk";
...
import { Percent as Percent3 } from "@pancakeswap/sdk";

// evm/stableSwap/getLPOutput.ts
import { CurrencyAmount as CurrencyAmount2, JSBI as JSBI3, ZERO as ZERO2 } from "@pancakeswap/sdk";
import invariant3 from "tiny-invariant";

// evm/stableSwap/amm.ts
import { JSBI, ZERO, ONE } from "@pancakeswap/sdk";
...
function getD({ amplifier, balances }) {
  ...
  const sum = balances.reduce((s, cur) => JSBI.add(s, JSBI.BigInt(cur)), ZERO);
...
import { CurrencyAmount, ERC20Token, Percent, Price, JSBI as JSBI2 } from "@pancakeswap/sdk";
...
function getFeePercent(inputAmount, outputAmount, { fee, adminFee }) {
  invariant2(fee.currency.equals(outputAmount.currency), "FEE_CURRENCY_MATCH");
  invariant2(adminFee.currency.equals(outputAmount.currency), "FEE_CURRENCY_MATCH");
...
var PRECISION = JSBI2.exponentiate(JSBI2.BigInt(10), JSBI2.BigInt(18));
var getRawAmount = (amount) => {
  return JSBI2.divide(
    JSBI2.multiply(amount.quotient, PRECISION),
    JSBI2.exponentiate(JSBI2.BigInt(10), JSBI2.BigInt(amount.currency.decimals))
  );
};
var parseAmount = (currency, rawAmount) => {
  return CurrencyAmount.fromRawAmount(
    currency,
    JSBI2.divide(
      JSBI2.multiply(rawAmount, JSBI2.exponentiate(JSBI2.BigInt(10), JSBI2.BigInt(currency.decimals))),
      PRECISION
    )
  );
};
...
function getLPOutput({
  ...
  if (JSBI3.equal(lpTotalSupply, ZERO2) || !balances.length || balances.every((b) => JSBI3.equal(b.quotient, ZERO2))) {
    const d = getD({ amplifier, balances: amounts.map(getRawAmount) });
    return CurrencyAmount2.fromRawAmount(lpToken, d);
  }
  const currentBalances = [];
  const newBalances = [];
  for (const [i, balance] of balances.entries()) {
    const amount = amounts[i] || CurrencyAmount2.fromRawAmount(balance.currency, 0);
    invariant3(
    ...
    newBalances.push(JSBI3.add(balanceRaw, amountRaw));
  }
  const d0 = getD({ amplifier, balances: currentBalances });
  const d1 = getD({ amplifier, balances: newBalances });
  invariant3(JSBI3.greaterThanOrEqual(d1, d0), "D1 should be greater than or equal than d0.");
  const isFirstSupply = JSBI3.lessThanOrEqual(lpTotalSupply, ZERO2);
  if (isFirstSupply) {
    return CurrencyAmount2.fromRawAmount(totalSupply.currency, d1);
  }
  const n = currentBalances.length;
  const eachTokenFee = fee.multiply(n).divide(4 * (n - 1));
  let d2 = d1;
  for (const [i, b] of currentBalances.entries()) {
    const idealBalance = JSBI3.divide(JSBI3.multiply(d1, b), d0);
    let diff = ZERO2;
    if (JSBI3.greaterThan(idealBalance, newBalances[i])) {
      diff = JSBI3.subtract(idealBalance, newBalances[i]);
    } else {
      diff = JSBI3.subtract(newBalances[i], idealBalance);
    }
    const feeAmount = eachTokenFee.multiply(diff).quotient;
    newBalances[i] = JSBI3.subtract(newBalances[i], feeAmount);
  }
  d2 = getD({ amplifier, balances: newBalances });
  const expectedMintLP = JSBI3.divide(JSBI3.multiply(lpTotalSupply, JSBI3.subtract(d2, d0)), d0);
  return CurrencyAmount2.fromRawAmount(totalSupply.currency, expectedMintLP);
}

// evm/stableSwap/getLPOutputWithoutFee.ts
function getLPOutputWithoutFee(params) {
  return getLPOutput(__spreadProps(__spreadValues({}, params), { fee: new Percent3(0) }));
}

// evm/stableSwap/getSwapOutput.ts
import { CurrencyAmount as CurrencyAmount4, JSBI as JSBI4, Percent as Percent4, ZERO as ZERO3, ONE_HUNDRED_PERCENT } from "@pancakeswap/sdk";
...
function getSwapOutput({
  ...
  if (JSBI4.lessThan(amount.quotient, ZERO3)) {
    ...
    const dy2 = JSBI4.subtract(y2, balances[j]);
    return parseAmount(outputCurrency, dy2);
  }
  const y = getY({ amplifier, balances, i, j, x: getRawAmount(amount) });
  const dy = JSBI4.subtract(balances[j], y);
  const feeAmount = fee.multiply(dy).quotient;
  return parseAmount(outputCurrency, JSBI4.subtract(dy, feeAmount));
}
function getSwapOutputWithoutFee(params) {
  return getSwapOutput(__spreadProps(__spreadValues({}, params), { fee: new Percent4(0) }));
}
...
import { ChainId as ChainId4 } from "@pancakeswap/sdk";

// ../tokens/src/common.ts
import { ChainId, ERC20Token as ERC20Token2 } from "@pancakeswap/sdk";
var CAKE_MAINNET = new ERC20Token2(
  ChainId.BSC,
...
var CAKE_TESTNET = new ERC20Token2(
  ChainId.BSC_TESTNET,
...
var USDC_TESTNET = new ERC20Token2(
  ChainId.BSC_TESTNET,
...
var USDC_GOERLI = new ERC20Token2(
  ChainId.GOERLI,
...
var BUSD_TESTNET = new ERC20Token2(
  ChainId.BSC_TESTNET,
...
var BUSD_GOERLI = new ERC20Token2(
  ChainId.GOERLI,
...
// ../tokens/src/56.ts
import { ChainId as ChainId2, WBNB, ERC20Token as ERC20Token3 } from "@pancakeswap/sdk";
var bscTokens = {
  wbnb: WBNB[ChainId2.BSC],
  ...
  qi: new ERC20Token3(ChainId2.BSC, "0x8729438EB15e2C8B576fCc6AeCdA6A148776C0F5", 18, "QI", "BENQI", "https://benqi.fi/"),
  sheesha: new ERC20Token3(
    ChainId2.BSC,
  ...
  co: new ERC20Token3(ChainId2.BSC, "0x936B6659Ad0C1b244Ba8Efe639092acae30dc8d6", 6, "CO", "CO", "https://corite.com/"),
  krs: new ERC20Token3(
    ChainId2.BSC,
...
// ../tokens/src/97.ts
import { ChainId as ChainId3, ERC20Token as ERC20Token4, WBNB as WBNB2 } from "@pancakeswap/sdk";
var bscTestnetTokens = {
  wbnb: WBNB2[ChainId3.BSC_TESTNET],
  cake: CAKE_TESTNET,
  busd: BUSD_TESTNET,
  syrup: new ERC20Token4(
    ChainId3.BSC_TESTNET,
  ...
  hbtc: new ERC20Token4(ChainId3.BSC_TESTNET, "0x3Fb6a6C06c7486BD194BB99a078B89B9ECaF4c82", 18, "HBTC", "Huobi BTC"),
  wbtc: new ERC20Token4(ChainId3.BSC_TESTNET, "0xfC8bFbe9644e1BC836b8821660593e7de711e564", 8, "WBTC", "Wrapped BTC"),
  usdc: new ERC20Token4(
    ChainId3.BSC_TESTNET,
  ...
  usdt: new ERC20Token4(ChainId3.BSC_TESTNET, "0x0fB5D7c73FA349A90392f873a4FA1eCf6a3d0a96", 18, "USDT", "Tether USD"),
  mockBusd: new ERC20Token4(
    ChainId3.BSC_TESTNET,
...
var STABLE_SUPPORTED_CHAIN_IDS = [ChainId4.BSC, ChainId4.BSC_TESTNET];
var STABLE_POOL_MAP = {
  [ChainId4.BSC]: pools,
  [ChainId4.BSC_TESTNET]: pools2
};
      ...
      return createStableSwapPair(
        {
          token0,
          token1,
          reserve0: CurrencyAmount5.fromRawAmount(token0, "0"),
          reserve1: CurrencyAmount5.fromRawAmount(token1, "0")
        },
        stableSwapAddress,
        lpAddress,
        infoStableSwapAddress,
...
import { CurrencyAmount as CurrencyAmount7, Pair as Pair2 } from "@pancakeswap/sdk";
import { createMulticall } from "@pancakeswap/multicall";

// evm/utils/currency.ts
import { CurrencyAmount as CurrencyAmount6, Native, WNATIVE } from "@pancakeswap/sdk";
function wrappedCurrency(currency, chainId) {
  return (currency == null ? void 0 : currency.isNative) ? WNATIVE[chainId] : (currency == null ? void 0 : currency.isToken) ? currency : void 0;
}
function wrappedCurrencyAmount(currencyAmount, chainId) {
  ...
  return token && currencyAmount ? CurrencyAmount6.fromRawAmount(token, currencyAmount.quotient) : void 0;
}
  ...
  return __async(this, arguments, function* (currencyPairs, { provider, chainId }) {
    ...
    const resultWithState = results.map((result, i) => {
      ...
      return [
        2 /* EXISTS */,
        new Pair2(
          CurrencyAmount7.fromRawAmount(token0, reserve0.toString()),
          CurrencyAmount7.fromRawAmount(token1, reserve1.toString())
        )
      ];
    });
...
function getPairAddress([tokenA, tokenB]) {
  let addr = "";
  try {
    addr = tokenA && tokenB && !tokenA.equals(tokenB) ? Pair2.getAddress(tokenA, tokenB) : "";
  } catch (error) {
...
import { CurrencyAmount as CurrencyAmount8 } from "@pancakeswap/sdk";
import { createMulticall as createMulticall2 } from "@pancakeswap/multicall";
...
function getStableSwapOutputAmount(_0, _1, _2) {
  return __async(this, arguments, function* (pair, inputAmount, { provider }) {
    const { multicallv2 } = createMulticall2(provider);
    ...
    return CurrencyAmount8.fromRawAmount(outputToken, result.toString());
  });
}

// evm/onchain/getStableSwapFee.ts
import { createMulticall as createMulticall3 } from "@pancakeswap/multicall";
import { CurrencyAmount as CurrencyAmount9 } from "@pancakeswap/sdk";
...
import { JSBI as JSBI5, Percent as Percent5 } from "@pancakeswap/sdk";
var BIG_INT_TEN = JSBI5.BigInt(10);
var BIPS_BASE = JSBI5.BigInt(1e4);
var MIN_BNB = JSBI5.exponentiate(BIG_INT_TEN, JSBI5.BigInt(16));
var BETTER_TRADE_LESS_HOPS_THRESHOLD = new Percent5(JSBI5.BigInt(50), BIPS_BASE);

// evm/constants/exchange.ts
import { ChainId as ChainId9, WBNB as WBNB3, WNATIVE as WNATIVE2 } from "@pancakeswap/sdk";
var ROUTER_ADDRESS = {
  [ChainId9.ETHEREUM]: "0x3BC722f252C7bAE2f55647e49aDcB9d33Ff6eBcC",
  [ChainId9.GOERLI]: "0x3BC722f252C7bAE2f55647e49aDcB9d33Ff6eBcC",
  [ChainId9.BSC]: "0x10ED43C718714eb63d5aA57B78B54704E256024E",
  [ChainId9.BSC_TESTNET]: "0xD99D1c33F9fC3444f8101754aBC46c52416550D1"
};
var STABLE_SWAP_INFO_ADDRESS = {
  [ChainId9.ETHEREUM]: "",
  [ChainId9.GOERLI]: "",
  [ChainId9.BSC]: "0xa680d27f63Fa5E213C502d1B3Ca1EB6a3C1b31D6",
  [ChainId9.BSC_TESTNET]: "0xaE6C14AAA753B3FCaB96149e1E10Bc4EDF39F546"
};
var BASES_TO_CHECK_TRADES_AGAINST = {
  [ChainId9.ETHEREUM]: [
    WNATIVE2[ChainId9.ETHEREUM],
    USDC[ChainId9.ETHEREUM],
    USDT[ChainId9.ETHEREUM],
    BUSD[ChainId9.ETHEREUM],
    WBNB3[ChainId9.ETHEREUM]
  ],
  [ChainId9.GOERLI]: [WNATIVE2[ChainId9.GOERLI], USDC[ChainId9.GOERLI], BUSD[ChainId9.GOERLI]],
  [ChainId9.BSC]: [
    bscTokens.wbnb,
    bscTokens.cake,
    bscTokens.busd,
  ...
  [ChainId9.BSC_TESTNET]: [bscTestnetTokens.wbnb, bscTestnetTokens.cake, bscTestnetTokens.busd]
};
...
var SUGGESTED_BASES = {
  [ChainId9.ETHEREUM]: [USDC[ChainId9.ETHEREUM], WBNB3[ChainId9.ETHEREUM], BUSD[ChainId9.ETHEREUM], USDT[ChainId9.ETHEREUM]],
  [ChainId9.GOERLI]: [USDC[ChainId9.GOERLI], WNATIVE2[ChainId9.GOERLI], BUSD[ChainId9.GOERLI]],
  [ChainId9.BSC]: [bscTokens.busd, bscTokens.cake, bscTokens.btcb],
  [ChainId9.BSC_TESTNET]: [bscTestnetTokens.wbnb, bscTestnetTokens.cake, bscTestnetTokens.busd]
};
var BASES_TO_TRACK_LIQUIDITY_FOR = {
  ...
  [ChainId9.GOERLI]: [USDC[ChainId9.GOERLI], WNATIVE2[ChainId9.GOERLI], BUSD[ChainId9.GOERLI]],
  [ChainId9.BSC]: [bscTokens.wbnb, bscTokens.dai, bscTokens.busd, bscTokens.usdt, bscTokens.cake],
  [ChainId9.BSC_TESTNET]: [bscTestnetTokens.wbnb, bscTestnetTokens.cake, bscTestnetTokens.busd]
};
var PINNED_PAIRS = {
  [ChainId9.ETHEREUM]: [
    [WNATIVE2[ChainId9.ETHEREUM], USDC[ChainId9.ETHEREUM]],
    [WBNB3[ChainId9.ETHEREUM], USDC[ChainId9.ETHEREUM]],
    [WBNB3[ChainId9.ETHEREUM], BUSD[ChainId9.ETHEREUM]],
    [WBNB3[ChainId9.ETHEREUM], USDT[ChainId9.ETHEREUM]],
    [WBNB3[ChainId9.ETHEREUM], WNATIVE2[ChainId9.ETHEREUM]]
  ],
  [ChainId9.BSC]: [
...
function getStableSwapFee(_0, _1, _2) {
  return __async(this, arguments, function* (pair, inputAmount, { provider }) {
    const chainId = inputAmount.currency.chainId;
    const { multicallv2 } = createMulticall3(provider);
    ...
    return {
      fee: CurrencyAmount9.fromRawAmount(outputToken, feeRaw.toString()),
      adminFee: CurrencyAmount9.fromRawAmount(outputToken, adminFeeRaw.toString())
    };
  });
}
  ...
  return __async(this, arguments, function* (amountIn, pairs, { provider }) {
    ...
    for (const pair of pairs) {
      ...
      pairsWithPrice.push(__spreadProps(__spreadValues({}, pair), {
        price: new Price2({ baseAmount: inputAmount, quoteAmount: outputAmount.add(fees.fee) }),
        fee,
        adminFee
      }));
    }
    return createTradeWithStableSwap({
      ...
      tradeType: TradeType2.EXACT_INPUT
    });
  });
}
...
import { Trade as Trade2, TradeType as TradeType3, isTradeBetter } from "@pancakeswap/sdk";
...
// evm/getBestTradeFromV2.ts
var getBestTradeFromV2ExactIn = createGetBestTradeFromV2(TradeType3.EXACT_INPUT);
var getBestTradeFromV2ExactOut = createGetBestTradeFromV2(TradeType3.EXACT_OUTPUT);
function createGetBestTradeFromV2(tradeType) {
  function getBestTrade(pairs, amountIn, output, options) {
    if (tradeType === TradeType3.EXACT_INPUT) {
      return Trade2.bestTradeExactIn(pairs, amountIn, output, options);
    }
    return Trade2.bestTradeExactOut(pairs, output, amountIn, options);
  }
...
import { CurrencyAmount as CurrencyAmount12, Price as Price3 } from "@pancakeswap/sdk";
function getBestTradeWithStableSwap(baseTrade, stableSwapPairs, options) {
  return __async(this, null, function* () {
    ...
    for (const [index, pair] of route.pairs.entries()) {
      ...
      if (stableSwapPair) {
        ...
        pairsWithStableSwap.push(__spreadProps(__spreadValues({}, stableSwapPair), {
          price: new Price3({ baseAmount: stableInputAmount, quoteAmount: outputAmount.add(fees.fee) }),
          fee,
          adminFee
        }));
    ...
    return createTradeWithStableSwap({
      ...
      outputAmount: CurrencyAmount12.fromFractionalAmount(
...
// evm/getBestTrade.ts
var getBestTradeExactIn = createGetBestTrade(TradeType5.EXACT_INPUT);
var getBestTradeExactOut = createGetBestTrade(TradeType5.EXACT_OUTPUT);
function createGetBestTrade(tradeType) {
  const isExactIn = tradeType === TradeType5.EXACT_INPUT;
...
import {
  TradeType as TradeType6,
  ZERO as ZERO4,
  CurrencyAmount as CurrencyAmount14,
  Fraction,
  ONE as ONE2,
  Price as Price5,
  computePriceImpact
...
// evm/utils/route.ts
import { Price as Price4 } from "@pancakeswap/sdk";
function getMidPrice(route) {
  ...
  for (const [i, pair] of route.pairs.entries()) {
    ...
    prices.push(
      route.path[i].wrapped.equals(pair.token0.wrapped) ? new Price4(pair.reserve0.currency, pair.reserve1.currency, pair.reserve0.quotient, pair.reserve1.quotient) : new Price4(pair.reserve1.currency, pair.reserve0.currency, pair.reserve1.quotient, pair.reserve0.quotient)
    );
  }
  const reduced = prices.slice(1).reduce((accumulator, currentValue) => accumulator.multiply(currentValue), prices[0]);
  return new Price4(route.input, route.output, reduced.denominator, reduced.numerator);
}
...
function maximumAmountIn(trade, slippageTolerance) {
  invariant5(!slippageTolerance.lessThan(ZERO4), "SLIPPAGE_TOLERANCE");
  if (trade.tradeType === TradeType6.EXACT_INPUT) {
    return trade.inputAmount;
  }
  const slippageAdjustedAmountIn = new Fraction(ONE2).add(slippageTolerance).multiply(trade.inputAmount.quotient).quotient;
  return CurrencyAmount14.fromRawAmount(trade.inputAmount.currency, slippageAdjustedAmountIn);
}
function minimumAmountOut(trade, slippageTolerance) {
  invariant5(!slippageTolerance.lessThan(ZERO4), "SLIPPAGE_TOLERANCE");
  if (trade.tradeType === TradeType6.EXACT_OUTPUT) {
    return trade.outputAmount;
  }
  const slippageAdjustedAmountOut = new Fraction(ONE2).add(slippageTolerance).invert().multiply(trade.outputAmount.quotient).quotient;
  return CurrencyAmount14.fromRawAmount(trade.outputAmount.currency, slippageAdjustedAmountOut);
}
function executionPrice({
  inputAmount,
  outputAmount
}) {
  return new Price5(inputAmount.currency, outputAmount.currency, inputAmount.quotient, outputAmount.quotient);
}
